...
<div class="mod_search clearfix">
    <h2><?php echo L('搜索');?><?php echo ($ts['app']['app_alias']); ?></h2>
  <div>
	<form action="<?php echo U('blog/Index/search');?>" method="GET" name="subform">
      <input name="app" type="hidden" value="blog"/>
	  <input name="mod" type="hidden" value="Index"/>
	  <input name="act" type="hidden" value="search"/>
	  <span class="left mr5">
	  <input name="key" type="text" value="<?php echo (t($_GET['key'])); ?>" class="txt_blog_s"/>
	  </span>
	  <input type="submit" class="btn_b left" value="<?php echo L('搜索');?>" />
	</form>
  </div>
</div>

<div class="mod_recommend clearfix">
    <h2><?php echo L('推荐');?><?php echo ($ts['app']['app_alias']); ?></h2>
    <?php if(empty($relist)){ ?><?php echo L('暂无推荐');?><?php echo ($ts['app']['app_alias']); ?><?php } ?>
        <ul>    
        <?php if(is_array($relist)): ?><?php $i = 0;?><?php $__LIST__ = $relist?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><a href="<?php echo U('blog/Index/show', array('id'=>$vo['id'], 'mid'=>$vo['uid']));?>"><?php echo ($vo["title"]); ?></a></li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
        </ul>    
</div>

<?php if((ACTION_NAME)  ==  "my"): ?><div class="mod_class clearfix">
    <h2><?php echo L('日志分类');?>（<span><a href="<?php echo U('blog/Index/admin');?>" style="padding-left: 0px;" ><?php echo L('管理');?></a></span>）</h2>
    <ul>
    <li><a href="<?php echo U('blog/Index/my');?>"><?php echo L('所有分类');?></a></li>
    <?php foreach( $category as $key=>$value ) { ?>
    <li><a href="<?php echo U('blog/Index/my', array('cateId'=>$value['id']));?>"><?php echo ($value['name']); ?> ( <?php echo ($value['count']); ?> )</a></li>
    <?php } ?>
    </ul>
</div>
<?php echo W( 'FileAway', $file_away);?><?php endif; ?>
</div>
  ...
  <ul class="tit_tab">
    <li><a href="<?php echo U('wiki/Index/index');?>" <?php if(ACTION_NAME=='index')echo 'class="on"'; ?>><?php echo ($ts['app']['app_alias']); ?>中心</a></li>
    <li><a href="<?php echo U('wiki/Index/mywiki');?>" <?php if(ACTION_NAME=='mywiki')echo 'class="on"'; ?>>我的<?php echo ($ts['app']['app_alias']); ?></a></li>
  </ul>
  <div class="c"></div>
</div>
