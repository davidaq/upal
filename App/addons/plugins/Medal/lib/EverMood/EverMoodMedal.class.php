 ...
 */
class EverMoodMedal extends BaseMedal {
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( intval($user_medal_data['data']['received_time']) <= 0 && $this->__isEverMood($uid) ) {
	...
	private function __isEverMood($uid, $days = 3) {
		$count=M('mood')->where('uid="'.$uid.'"')->count();
		if($count>14){
